...
from methods.GPAWP.utils.data import load_pretrain_data,load_data_IMDB,load_pretrain_data_IMDB
...
def hprompt_pretrain_sample(dataset,dl,adj,n_unrelated):
    ...
    if dataset=="ACM":
        ...
        BOOK_nodes = np.arange(dl.nodes['shift'][0], dl.nodes['shift'][0] + dl.nodes['count'][0])
        target_node_tuple_num = np.zeros_like(BOOK_nodes)
        whole = []
        whole.append(BOOK_nodes)
        res = None
        first_res = True
        ##each relation
        count = 0
...
def run_model_DBLP(args):
    ...
    # if args.dataset=="IMDP":
    #     features_list, adjM, labels, train_val_test_idx, dl = load_pretrain_data_IMDB
    # else:
    ...
    samples_dir=os.path.join('/home/liusy/GPAWP/data',args.dataset,"".join([str(args.tuple_neg_disconnected_num),'neg_samples']))
    load_samples_dir=os.path.join('/home/liusy/GPAWP/data',args.dataset,"".join([str(args.tuple_neg_disconnected_num),'neg_samples.npy']))
    ...
    samples=torch.tensor(samples,dtype=int).to(device)
            ...
            if args.hetero_pretrain:
                ...
                if args.each_loss:
                    ...
                    if args.target_pretrain:
                        ...
                        train_loss = compareloss(logits, samples.to(device), args.temperature, device)
