...
class acm_hnode_semantic_prompt_layer_feature_weighted_sum(nn.Module):
    ...
    def save_semantic_weight(self):
        with torch.no_grad():
            return self.semantic_weight.clone().detach()

    def semantic_weight_return(self, weight):
        with torch.no_grad():
            self.semantic_weight.data.copy_(weight)
    ...
    def weight_return(self, weight):
        with torch.no_grad():
            self.weight.data.copy_(weight)



    def mask_weight_token(self, mask):

        mask = mask.unsqueeze(0)  # [1, num_tokens]
        with torch.no_grad():
        ...
        if hasattr(self, '_mask_hook') and self._mask_hook is not None:
            self._mask_hook.remove()
        self._mask_hook = self.weight.register_hook(grad_mask_hook)


    def mask_semantic_token(self, mask):
        with torch.no_grad():
            self.semantic_weight.data *= mask.unsqueeze(0)
...
class freebase_bidirection_semantic_hnode_prompt_layer_feature_weighted_sum(nn.Module):
    ...
    def mask_weight_token(self, mask):
        with torch.no_grad():
            self.weight.data *= mask.unsqueeze(0)
...
class imdb_hnode_semantic_prompt_layer_feature_weighted_sum(nn.Module):
    def __init__(self,
                 input_dim,
                 negative_slope=0.2,
                 semantic_prompt_weight=0.1
                 ):
        super(imdb_hnode_semantic_prompt_layer_feature_weighted_sum, self).__init__()
        self.weight = torch.nn.Parameter(torch.Tensor(1, input_dim))
        self.semantic_weight = torch.nn.Parameter(torch.Tensor(1, 5))  # 1 global + 4 edge types
        self.leaky_relu = nn.LeakyReLU(negative_slope)
        self.semantic_prompt_weight = semantic_prompt_weight
        self.reset_parameters()
        self._mask_hook = None  # for gradient masking
...
class imdb_hnode_semantic_prompt_layer_feature_weighted_sum_twohop(nn.Module):
    def __init__(self,
                 input_dim,
                 negative_slope=0.2,
                 semantic_prompt_weight=0.1
                 ):
        super(imdb_hnode_semantic_prompt_layer_feature_weighted_sum_twohop, self).__init__()

        self.weight = nn.Parameter(torch.Tensor(1, input_dim))
        self.semantic_weight = nn.Parameter(torch.Tensor(1, 5))
        self.leaky_relu = nn.LeakyReLU(negative_slope)
        self.semantic_prompt_weight = semantic_prompt_weight
    ...
    def reset_parameters(self):
        ...
        nn.init.xavier_uniform_(self.semantic_weight)
    ...
    def message_func_twohop(self, edges):
        ...
        mask0 = (edges.data['e'] == 0).float().view(-1, 1)  # [E,1]
        res0 = edges.src['ft0'] * mask0

        mask1 = (edges.data['e'] == 1).float().view(-1, 1)
        res1 = edges.src['ft1'] * mask1

        return {'m': res + res0 + res1}

    def message_func_twohop_semantic(self, edges):
        sem = F.normalize(self.semantic_weight, p=2, dim=1)


        res = edges.src['ft'] * sem[0, 0]

        mask0 = (edges.data['e'] == 0).float().view(-1, 1)
        res0 = edges.src['ft0'] * mask0 * sem[0, 1]

        mask1 = (edges.data['e'] == 1).float().view(-1, 1)
        res1 = edges.src['ft1'] * mask1 * sem[0, 2]

        return {'m': res + res0 + res1}

    def message_func_onehop(self, edges):

        res = edges.src['ft_twohop']
        mask2 = (edges.data['e'] == 2).float().view(-1, 1)
        res2 = edges.src['ft2'] * mask2

        mask3 = (edges.data['e'] == 3).float().view(-1, 1)
        res3 = edges.src['ft3'] * mask3

        return {'m': res + res2 + res3}
    ...
    def message_func_onehop_semantic(self, edges):
        ...
        res = edges.src['ft_twohop'] * sem[0, 3]

        mask2 = (edges.data['e'] == 2).float().view(-1, 1)
        res2 = edges.src['ft2'] * mask2 * sem[0, 4]

        mask3 = (edges.data['e'] == 3).float().view(-1, 1)
        res3 = edges.src['ft3'] * mask3 * sem[0, 4]
    ...
    def forward(self, graph, graph_embedding, e_feat):
        ...
        base = F.elu(graph_embedding * self.weight)  # [N, D]
        ...
        graph.update_all(self.message_func_onehop, fn.sum('m', 'ft_onehop'))
        graph.update_all(self.message_func_onehop_semantic, fn.sum('m', 'ft_onehop_s'))
        res = graph.ndata['ft_onehop'] + self.semantic_prompt_weight * graph.ndata['ft_onehop_s']
        return res
...
def multi_center_embedding(input, labels, label_num=0, debug=False):
    device = input.device
    ...
    count = labels.sum(dim=0, dtype=torch.float).unsqueeze(1)
