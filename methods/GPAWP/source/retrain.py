...
from methods.GPAWP.utils.data import load_data,load_pretrain_data_IMDB
...
from prompt import hnode_prompt_layer_feature_weighted_sum,node_prompt_layer_feature_weighted_sum,distance2center,center_embedding\
   ...
   ,dblp_hnode_prompt_layer_feature_weighted_sum\
    ...
    ,multi_center_embedding,imdb_hnode_semantic_prompt_layer_feature_weighted_sum,imdb_hnode_semantic_prompt_layer_feature_weighted_sum_twohop
...
def load_best_mask(args):
    path = './checkpoint/best_mask.pt'

    if not args.use_best_mask:
        return None
    ...
    semantic_mask = torch.tensor(data['semantic_mask']).to(args.device)
    weight_full = torch.tensor(data['weight_full']).to(args.device)

    print("Loaded mask from", path)

    return semantic_mask, weight_full
...
def run_model_DBLP(args):
    ...
    #         if not features_list[i].is_sparse:
    #             noise = torch.randn_like(features_list[i]) * args.feature_noise
    #             features_list[i] = features_list[i] + noise
    ...
    ##################### end of if freebase#############
                    ...
                    if args.each_type_subgraph:
                        if args.dataset == 'ACM':
                            ...
                            if args.semantic_prompt == 1:
                                classify = imdb_hnode_semantic_prompt_layer_feature_weighted_sum_twohop(hidden_dim,
                                                                                                        semantic_prompt_weight=args.semantic_prompt_weight)
            ...
            early_stopping_classify = EarlyStopping(patience=args.patience, verbose=True,
                                           save_path='./checkpoint/retrain/checkpoint_{}_{}_{}_{}_freeze_classify.pt'.format(args.dataset,args.model_type,args.tuning,args.shotnum))


            classify.load_state_dict(torch.load('./checkpoint/checkpoint_{}_{}_{}_{}_freeze_classify.pt'.format(args.dataset, args.model_type,args.tuning,args.shotnum)))
            ...
            for epoch in range(args.epoch):
                ...
                loaded_mask = load_best_mask(args.use_best_mask)

                if epoch == 0:

                    if loaded_mask is not None:
                        semantic_mask, weight_full = loaded_mask
                        classify.mask_semantic_token(semantic_mask)


                        try:
                            classify.mask_weight_token(weight_full)
                        except:
                            sub_dim = args.hidden_dim // args.num_subweight

                            for i in range(args.num_subweight):
                                sub = weight_full[i * sub_dim:(i + 1) * sub_dim]
                                classify.mask_weight_token(sub)

                    else:
                        if args.dataset == 'Freebase':
                            inputmask = torch.tensor([1, 0, 0, 1, 0, 0, 0, 0, 0]).to(device)
                        else:
                            inputmask = torch.tensor([1, 1, 0, 1, 1]).to(device)

                        classify.mask_semantic_token(inputmask)

                        weightmask = torch.tensor([
                            [0] * 4 + [1] * 60,
                            [1] * 20 + [0] * 4 + [1] * 40,
                            [1] * 28 + [0] * 4 + [1] * 32,
                            [1] * 32 + [0] * 4 + [1] * 28,
                        ...
                        for mask in weightmask:
                            classify.mask_weight_token(mask)
                ...
                if args.add_edge_info2prompt:
                    ...
                    'weight-sum', 'weight-sum-center-fixed', 'bottle-net', 'sum', 'cat', 'cat_edge', 'prompt_gcn'):
                        if args.dataset == 'Freebase':
                            if args.freebase_type == 2:
                    ...
                    if args.tuning in (
                    'gcn', 'weighted-sum', 'weight-sum-center-fixed', 'bottle-net', 'cat', 'sum', 'cat_edge',
                    'prompt_gcn'):
                        logits = classify(g, prelogits)
                ...
                if args.tuning in ('gcn', 'linear'):
                    ...
                    train_loss = F.nll_loss(logp, train_labels.long())
                ...
                with torch.no_grad():
                    ...
                    if args.add_edge_info2prompt:
                        ...
                        'weight-sum', 'weight-sum-center-fixed', 'bottle-net', 'cat', 'sum', 'cat_edge', 'prompt_gcn'):
                            if args.dataset == 'Freebase':
                                if args.freebase_type == 2:
                                    logits = classify(g, trans_g, prelogits, e_feat)
                                else:
                    ...
                    if args.tuning in ('weight-sum-center-fixed', 'cat', 'bottle-net', 'sum', 'cat_edge', 'prompt_gcn'):
                        ...
                        if args.pretrain_each_loss:
                            ...
                            # distance = 1 / F.normalize(distance, dim=1)
                            logp = F.log_softmax(distance, dim=1)
                            val_loss = F.nll_loss(logp, train_labels.long())
            ...
            if args.tuning == 'weight-sum-center-fixed':
                ...
                np.save('./checkpoint/retrain/{}_task_center_embedding'.format(count), save_c_emb)
            ...
            classify.load_state_dict(torch.load(
                './checkpoint/retrain/checkpoint_{}_{}_{}_{}_freeze_classify.pt'.format(
            ...
            with torch.no_grad():
                ...
                if args.tuning == 'gcn':
                    ...
                    load_c_emb = np.load('./checkpoint/retrain/{}_task_center_embedding.npy'.format(count), allow_pickle=True)
    ...
    total_params = sum(p.numel() for p in classify.parameters() if p.requires_grad)
    ...
    with open("./checkpoint/retrain-fewshot.txt", "a") as file:
        print("\n",file=file)
        print("shotnum", args.shotnum,file =file)
        print("lr", args.lr, file=file)
        print("epoch", args.epoch, file=file)
        print("patience", args.patience, file=file)
        print("dataset",args.dataset,file=file)
        print('microf mean: ', np.mean(eval_result['micro-f1']), 'acc std: ', np.std(eval_result['micro-f1']),
              file=file)
        print('macroF mean: ', np.mean(eval_result['macro-f1']), 'macroF std: ', np.std(eval_result['macro-f1']),
              file=file)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--seed', type=int, default="1")  #999 for freebase
    ...
    ap.add_argument('--freebase-type', type=int, default=2, help='0:book as source, 1:book as destination, 2: bidirection')
    ...
    ap.add_argument('--feature-noise', type=float, default=0,
                    help='Gaussian noise level for node features')
    ap.add_argument('--edge-noise', type=float, default=0,
                    help='Proportion of edges to perturb (0~1)')
    ap.add_argument(
        "--use_best_mask",
        action="store_true",
        help="Whether to load the best mask or not"
    )
    args = ap.parse_args()
    run_model_DBLP(args)
