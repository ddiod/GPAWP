...
from prompt import hnode_prompt_layer_feature_weighted_sum,node_prompt_layer_feature_weighted_sum,distance2center,center_embedding\
    ...
    , imdb_hnode_semantic_prompt_layer_feature_weighted_sum_twohop
...
def run_model_DBLP(args):
    ...
    ##################### end of if freebase#############
                ...
                if args.add_edge_info2prompt:
                    ...
                    if args.each_type_subgraph:
                        if args.dataset=='ACM':
                            ...
                            if args.semantic_prompt == 1:
                                # classify= imdb_hnode_semantic_prompt_layer_feature_weighted_sum(hidden_dim,semantic_prompt_weight=args.semantic_prompt_weight)
                                classify = imdb_hnode_semantic_prompt_layer_feature_weighted_sum_twohop(hidden_dim,semantic_prompt_weight=args.semantic_prompt_weight)
            ...
            early_stopping_classify = EarlyStopping(patience=args.patience, verbose=True,
                                           save_path='./checkpoint/checkpoint_{}_{}_{}_{}_freeze_classify.pt'.format(args.dataset,
            ...
            for epoch in range(args.epoch):
                ...
                if epoch % 3 == 0:
                    with open('./checkpoint/loss.txt', 'a') as losfil:
                        print('Epoch {:05d} | Train_Loss {:.4f} |  Time(s) {:.4f}'.format(
                            epoch, train_loss.item(), t_end - t_start),file=losfil)
                ...
                if epoch % 3 == 0:
                    with open('./checkpoint/loss.txt', 'a') as losfil:
                        print('Epoch {:05d} | Val_Loss {:.4f} | Time(s) {:.4f}'.format(
                            epoch, val_loss.item(), t_end - t_start),file=losfil)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--besteval',type=int,default=20,help='number of  best test F1')
    ap.add_argument('--bestevalma',type=int,default=10)
    ap.add_argument('--feature-noise', type=float, default=0.05,
